...
export default function CreateReportScreen({ route, navigation }) {
  ...
  const [userDefinedIssueType, setUserDefinedIssueType] = useState(""); // New state
  ...
  const canSubmit = useMemo(
    () =>
      ...
      !submitting &&
      !(issueType === "other" && !userDefinedIssueType.trim()), // New validation
      ...
      try {
        ...
        if (status !== "granted") {
          Alert.alert(
            ...
            "You can still submit a report, but location data will not be included."
          );
          setLatitude(null);
          setLongitude(null);
        } else {
  ...
  async function submit() {
    ...
    try {
      await createUserReport({
        ...
        lat: latitude,
        lon: longitude,
        user_defined_issue_type: issueType === "other" ? userDefinedIssueType.trim() : undefined, // Pass conditionally
        details: details.trim() || undefined, // Pass if not empty
      });

      // Go back to feed and ask it to refresh
      navigation.reset({
        index: 0,
        routes: [{ name: "Feed", params: { refresh: true } }],
      });
  ...
  return (
    ...
    >
      ...
      <View style={styles.section}>
        ...
        {issueType === "other" && (
          <TextInput
            placeholder="Name your issue (e.g., 'Broken Streetlight')"
            value={userDefinedIssueType}
            onChangeText={setUserDefinedIssueType}
